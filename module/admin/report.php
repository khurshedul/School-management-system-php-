...
$sql = "select t.name as teacher,ac.name as course,ac.classid as class,g.grade,count(g.id) as no_of_std from teachers t,takencoursebyteacher tc,availablecourse ac,grade g where t.id=tc.teacherid and ac.id=tc.courseid and tc.courseid=g.courseid and g.grade not in('A+','A','A-','B+','B','B-')group by g.grade";
$res = mysql_query($sql);
while($row = mysql_fetch_array($res)){
    $string .= "<tr><td>".$row['teacher']."</td><td>".$row['course']."</td><td>".$row['class'].
    "</td><td>".$row['grade']."</td><td>".$row['no_of_std']."</td></tr>";
}
?>
<html>
    <head>
    ...
    <body>
			...
			<br/><br/>
			  ...
			  <hr/>
        <center>
            <h1>Call This Teacher And Give Them Some Jhari.</h1>
            <table border="1">
                <?php echo $string;?>
            </table>
        </center>
